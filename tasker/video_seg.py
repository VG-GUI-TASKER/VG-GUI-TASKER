...
def split_and_reconnect_segments(selected_video_segments, video_segments, for_seg_not_interested, num_frames, split_point_fn=None):
    """
    ...
    :param split_point_fn: Optional callable(segment_start, segment_end) -> int. 
                           If provided, uses this function to compute the split point instead of midpoint.
    :return: List of new video segments
    """
    def get_split_point(seg_start, seg_end):
        if split_point_fn is not None:
            try:
                sp = split_point_fn(seg_start, seg_end)
                # Ensure the split point is strictly between start and end
                if seg_start < sp < seg_end:
                    return sp
            except Exception:
                pass
        return (seg_start + seg_end) // 2  # fallback to midpoint
    ...
    if for_seg_not_interested == "prune":
        ...
        for segment in selected_video_segments:
            ...
            if segment.start >= segment.end - 1:
                ...
                split_pt = get_split_point(segment.start, segment.end)

                # Create two new VideoSeg instances
                first_half = VideoSeg(start=segment.start, end=split_pt)
                second_half = VideoSeg(start=split_pt, end=segment.end)
